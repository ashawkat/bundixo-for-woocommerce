...
/**
 * Registers and serves the bundixo/v1 REST namespace.
 */
class Rest {
	...
	public function get_bundle( $request ) {
		...
		if ( ! $bundle ) {
			return new \WP_Error( 'bundixo_not_found', __( 'Bundle not found.', 'bundixo-for-woocommerce' ), [ 'status' => 404 ] );
		}
	...
	public function delete_bundle( $request ) {
		...
		if ( ! Bundles::delete( $bundle_id ) ) {
			return new \WP_Error( 'bundixo_delete_failed', __( 'Failed to delete bundle.', 'bundixo-for-woocommerce' ), [ 'status' => 500 ] );
		}
	...
	public function get_diagnostics() {
		...
		$diagnostics = [
			...
			'plugin_version'    => BUNDIXO_VERSION,
			'db_version'        => get_option( 'bundixo_db_version', '' ),
			'table_name'        => $table,
	...
	public function get_quote( $request ) {
		...
		if ( empty( $items ) ) {
			return new \WP_Error( 'bundixo_empty_quote', __( 'No valid products selected.', 'bundixo-for-woocommerce' ), [ 'status' => 400 ] );
		}
	...
	public function add_to_cart( $request ) {
		if ( ! function_exists( 'WC' ) || ! function_exists( 'wc_get_product' ) ) {
			return new \WP_Error( 'bundixo_wc_missing', __( 'WooCommerce is not available.', 'bundixo-for-woocommerce' ), [ 'status' => 500 ] );
		}
		...
		if ( empty( $items ) ) {
			return new \WP_Error( 'bundixo_empty_selection', __( 'No valid products selected.', 'bundixo-for-woocommerce' ), [ 'status' => 400 ] );
		}
		...
		if ( empty( $quote['products'] ) ) {
			return new \WP_Error( 'bundixo_empty_selection', __( 'No purchasable products selected.', 'bundixo-for-woocommerce' ), [ 'status' => 400 ] );
		}
		...
		if ( ! WC()->session || ! WC()->cart ) {
			return new \WP_Error( 'bundixo_cart_missing', __( 'Cart is not available.', 'bundixo-for-woocommerce' ), [ 'status' => 500 ] );
		}
		...
		foreach ( $grouped as $item ) {
			...
			if ( ! $product || ! $product->is_purchasable() ) {
				$failed[] = sprintf(
					...
					__( 'Product cannot be purchased: #%d', 'bundixo-for-woocommerce' ),
					$check_id
				);
				continue;
			}
			...
			$cart_item_data = [
				'bundixo_bundle_item' => true,
				'bundixo_bundle_id'   => $bundle['id'],
			];
			...
			if ( $cart_item_key ) {
				...
				$failed[] = sprintf(
					...
					__( 'Failed to add product #%d to the cart.', 'bundixo-for-woocommerce' ),
					$product_id
				);
			}
		}
		...
		if ( empty( $added ) ) {
			return new \WP_Error(
				'bundixo_add_failed',
				__( 'Failed to add any products to the cart.', 'bundixo-for-woocommerce' ),
		...
		do_action( 'bundixo_bundle_added_to_cart', $bundle['id'], $quote );
